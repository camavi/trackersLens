...
Nota aggiornata 2026-06-09: `flowMap.html` include ora una prima `AI Chat` per creare runtime graph da prompt. Il modulo `js/flow-map/flowMapPromptChat.js` legge i settings AI globali da `tl_settings`, usa il provider/modello selezionato per generare un piano JSON (`Ollama` via `/api/generate`, LM Studio/OpenAI-compatible via `/v1/chat/completions`), normalizza il risultato contro la palette Flow Map, controlla se nello workspace esistono gia nodi/agenti con caratteristiche equivalenti, marca le parti come riuso o creazione e poi salva nodi e collegamenti reali usando `TrackerLensRuntimeGraphStore` / `TrackerLensChannelRegistry`. La chat non e piu solo prompt singolo: salva sessioni e messaggi nello store IndexedDB `tl_flow_prompt_chats`, separati per workspace, includendo prompt utente, piani generati, risultati di materializzazione ed errori. Se il provider AI non risponde o restituisce JSON non valido, usa fallback locale e mostra il motivo nel messaggio del piano. Lo stile vive in `css/flow-map/prompt-chat.css` e il bottone e nella palette sinistra sopra `Create Node`.
...
Nota aggiornata 2026-05-31: la Flow Map e stata spezzata in partial ordinati per ridurre il peso dei file principali senza cambiare runtime o comportamento. `flowMap.html` carica ora `js/flow-map/flowMapState.js`, `flowMapInteractions.js`, `flowMapRuntimeTests.js`, `flowMapNodeBuilder.js`, `flowMapRuntimeNodes.js` e `flowMapCanvasInspector.js` prima del bootstrap `js/flowMapView.js`. `css/flowMap.css` e diventato un entrypoint con import verso `css/flow-map/shell-palette.css`, `node-builder.css`, `canvas-nodes.css`, `inspector-config.css` e `status-responsive.css`. Per nuove modifiche Flow Map usare il partial coerente invece di reinserire logica nel bootstrap.

Nota aggiornata 2026-05-31: la categoria `Sources` della Flow Map include ora source locali per media e file: `Image Source`, `Audio Source`, `File Source` e `Files Batch`, oltre a REST/WebSocket/RSS/Webhook/API/manual/text/IndexedDB. I nuovi source salvano configurazione nel runtime node, usano porte OUT dedicate (`image`, `audio`, `file`, `files`) e il source immagine mostra una drop-zone sulla card: click per selezionare un file locale, drag/drop per caricare, preview immediata e salvataggio in config.

Nota aggiornata 2026-06-01: le drop-zone inline sono state estese anche a `Audio Source`, `File Source` e `Files Batch`. Ogni source media/file permette click per selezione locale e drag/drop sulla card; audio mostra un player compatto, file singolo mostra nome/tipo salvati e batch file salva metadata/data URL per piu file.
...
- `js/flowMapView.js`: i componenti custom `Checkbox` ora renderizzano `_.Checkbox` CMSwift nel preview, nella card reale del canvas e nel dialog `Custom Node Settings`, mentre `Toggle` resta uno switch distinto.
- `css/flowMap.css`: il layout dei `Checkbox` custom e' allineato al pattern del toggle, con label a sinistra e controllo checkbox a destra.
- `js/flowMapView.js`: il catalogo `Components` del Node Builder include ora anche `Radio`, `Rating`, `Date` e `Time`, con rendering CMSwift in preview, canvas e settings e schema custom dedicato.
- `js/flowMapView.js`: il dialog `Field Settings` salva ora impostazioni base per componente: visibilita sul nodo, esposizione automatica come porta IN/OUT, icon, color, default value/state, options per Select/Radio e min/max/step per Slider/Rating. Le impostazioni entrano nel `formLayout` e vengono usate dal preview, dalla card reale e dal dialog `Custom Node Settings`.
- `js/flowMapView.js`: l'icona custom dei componenti nel Node Builder ora parte da `null`; se il campo Icon resta vuoto, il componente CMSwift usa la propria icona/default interno.
- `js/flowMapView.js`: nel blocco `General` del Node Builder, Category/Icon/Tone usano ora `_.Select` CMSwift con liste globali `FLOW_NODE_CATEGORY_OPTIONS`, `FLOW_NODE_ICON_OPTIONS` e `FLOW_NODE_TONE_OPTIONS`, riusabili da altri punti della Flow Map.
- `js/flowMapView.js`: nel blocco `General` del Node Builder anche Name e Subtype usano `_.Input` CMSwift. Il dialog del builder disattiva la chiusura da outside/backdrop per evitare che la selezione di item nei dropdown chiuda il modal.
- `data/material-icons-list.json` e `js/tl-material-icon-options.js`: aggiunta lista Material Icons locale derivata dal gist `AmirOfir/daee915574b1ba0d877da90777dc2181`. I select icone devono usare questa lista locale (`window.TrackerLensMaterialIconOptions` / `FLOW_NODE_ICON_OPTIONS`) e non dipendere da URL esterni runtime. Vedi `docs/icon-selects.md`.
- `css/flowMap.css` / `js/flowMapView.js`: il footer del dialog Node Builder usa ora tre stili azione dedicati e confinati al dialog: `Cancel` neutro, `Save Template` ciano e `Create/Save Node` oro.
- `css/flowMap.css`: il preview laterale del Node Builder ora ha overflow interno sul contenuto del nodo, evitando che form custom lunghi escano dall'aside destro.
- `js/flowMapView.js`: il Node Builder custom non e' piu solo form/config. Il blocco Runtime permette di scegliere `Form only`, `REST API`, `WebSocket` o `RSS Feed`, salvando endpoint/metodo/body/keep-open nel nodo custom. I custom node con runtime REST/WebSocket/RSS diventano root testabili dal play della card e pubblicano il payload sulle porte OUT.
- `js/flowMapView.js`: anche i custom node `Form only` sono ora root testabili. Il play della card emette i valori del form (`metadata.config` + layout campi) come payload `flow_live_custom_form` sulle porte OUT, permettendo di usare un form custom come sorgente manuale per altri nodi.
- `js/flowMapView.js`: corretto il riconoscimento dei custom node con runtime `form` esplicito, cosi il play compare anche quando il builder salva `runtimeConnector: "form"`.
- `js/flowMapView.js`: il play e' ora sempre visibile per qualunque custom node root, inclusi REST/WebSocket/RSS anche prima di configurare l'endpoint; eventuali dati mancanti vengono gestiti dal test runtime con errore operativo.
- `js/flowMapView.js`: le porte IN/OUT del Node Builder hanno ora un dialog `Port Settings`. Gli input definiscono tipo dati, required, descrizione e schema; gli output definiscono tipo dati e sorgente payload (`runtimeResult`, `formData`, `component`, `static`, `function/mapping`) con path/componente/mapping. Il live test risolve il payload per porta prima di emettere sugli OUT.
...
- 2026-05-29: rafforzato il contratto `RuntimeManifest`: ogni nodo normalizzato espone sempre almeno una porta `inputs` e una porta `outputs`. Se un nodo non dichiara porte, `core/runtime/runtime-manifest.js` assegna default espliciti; Flow Map normalizza anche i manifest caricati, il graph model legge le porte manifest e la UI mostra porte default anche per Lens, Action, Storage e Dev Preview.
