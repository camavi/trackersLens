window.TrackerLensRuntimeManifest = (() => {
  ...
  const NODE_TYPES = new Set(["source", "boxTracker", "processor", "aiAgent", "boxLens", "lens", "action", "storage"]);
  ...
  const normalizeManifest = (manifest = {}) => {
    ...
    return {
      ...
      subtype,
      category,
      inputs: Array.isArray(manifest.inputs) ? manifest.inputs.map((port) => normalizePort(port, category === "lens" ? "any" : "object")) : [],
      outputs: Array.isArray(manifest.outputs) ? manifest.outputs.map((port) => normalizePort(port, "object")) : [],
  ...
  const validateManifest = (manifest = {}) => {
    ...
    if (["processor", "aiAgent", "action", "storage", "lens", "boxLens"].includes(normalized.type) && !normalized.inputs.length) {
      warnings.push(`${normalized.type} has no input ports`);
    }
    if (["source", "boxTracker", "processor", "aiAgent"].includes(normalized.type) && !normalized.outputs.length) {
      warnings.push(`${normalized.type} has no output ports`);
    }
