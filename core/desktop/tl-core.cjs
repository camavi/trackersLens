...
const createTlCore = ({ appVersion = "0.0.0", platform = "unknown", mode = "production", featureFlags = {}, adapters = {} } = {}) => {
  ...
  const request = async (command, payload = {}) => {
    switch (command) {
      ...
      case "desktop.persistence.readAiRunRecords":
        if (!persistence?.readAiRunRecords) throw errorWithCode("Desktop persistence is unavailable", "PERSISTENCE_UNAVAILABLE");
        return persistence.readAiRunRecords(payload);
      case "desktop.persistence.readAiMemoryMatches":
        if (!persistence?.readAiMemoryMatches) throw errorWithCode("Desktop persistence is unavailable", "PERSISTENCE_UNAVAILABLE");
        return persistence.readAiMemoryMatches(payload);
      ...
      case "desktop.persistence.readLibrarySummaryPage":
        if (!persistence?.readLibrarySummaryPage) throw errorWithCode("Desktop persistence is unavailable", "PERSISTENCE_UNAVAILABLE");
        return persistence.readLibrarySummaryPage(payload);
      case "desktop.persistence.readWorkspaceEditorIndex":
        if (!persistence?.readWorkspaceEditorIndex) throw errorWithCode("Desktop persistence is unavailable", "PERSISTENCE_UNAVAILABLE");
        return persistence.readWorkspaceEditorIndex();
      case "desktop.persistence.readAiDevToolsSummary":
        if (!persistence?.readAiDevToolsSummary) throw errorWithCode("Desktop persistence is unavailable", "PERSISTENCE_UNAVAILABLE");
        return persistence.readAiDevToolsSummary(payload);
      case "desktop.persistence.readAiRuntimeCenterSummary":
        if (!persistence?.readAiRuntimeCenterSummary) throw errorWithCode("Desktop persistence is unavailable", "PERSISTENCE_UNAVAILABLE");
        return persistence.readAiRuntimeCenterSummary(payload);
      case "desktop.persistence.readAnalyticsSummary":
        if (!persistence?.readAnalyticsSummary) throw errorWithCode("Desktop persistence is unavailable", "PERSISTENCE_UNAVAILABLE");
        return persistence.readAnalyticsSummary();
