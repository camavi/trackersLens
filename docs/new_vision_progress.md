...
- 2026-06-09: Aggiunta la prima `AI Flow Chat` nella Flow Map. Il bottone vive nella palette sinistra sopra `Create Node`; il dialog accetta prompt in forma conversazionale, usa i settings AI globali da `tl_settings` per chiamare il provider/modello selezionato, genera un piano JSON Source/Task -> Orchestrator Agent -> AI Agent/Processor/Action/Storage/Lens/Preview, controlla i nodi e i collegamenti gia presenti nello workspace e materializza solo le parti mancanti tramite `TrackerLensRuntimeGraphStore` e `TrackerLensChannelRegistry`. La chat ha storico persistente workspace-scoped in `tl_flow_prompt_chats` con sessioni, prompt utente, piani, risultati di creazione e errori. Se il provider AI non risponde o non restituisce JSON valido, usa fallback locale senza bloccare l'utente. Questa e la base concreta per AI generated runtime graph/workspaces senza introdurre uno store parallelo.
...
- 2026-06-01: La palette `Actions` e' stata estesa come layer `Actions / Outputs`: `Telegram Message`, `WhatsApp Message`, `Email`, `Webhook POST`, `HTTP PUT/PATCH`, `Browser Notification`, `Discord Message` e `Slack Message`. `Save DB Record` e `Save File` sono esposti in `Storage`, usando il runtime di persistenza esistente. `TrackerLensActionRuntime` supporta ora subtype HTTP/Telegram/WhatsApp/Slack dedicati, header JSON e metodo PUT/PATCH/POST per i nodi write.
- 2026-06-01: Introdotto `Orchestrator Agent` nella Flow Map come nodo AI runtime centrale. Usa subtype `orchestrator`, porte `task`, `decision`, `action`, `done`, `error`, dialog settings dedicato CMSwift e runtime separato `core/runtime/orchestrator-agent-runtime.js`. Il runtime legge i nodi direttamente collegati in uscita, costruisce un piano decisionale tracciabile, emette gli step sui channel downstream, blocca target non permessi/non configurati o action esterne quando e' richiesta conferma, e registra decisioni/log nel bus e negli store AI/runtime.
- 2026-06-01: Aggiunto `core/runtime/node-execution-controller.js`, controller condiviso per capacita runtime dei nodi. Processor, Action, Storage, AI Agent e Orchestrator ora rispettano `maxConcurrentTasks`/`parallelJobs`, `queueLimit`, `timeoutMs` e `dropPolicy` (`queue`, `reject`, `latest`), emettendo eventi `node_busy`, `node_queued`, `node_overloaded` e `node_idle`. Le card Flow Map mostrano busy/queued/overloaded. Gli AI Agent hanno anche `Input Data Request`: prima del prompt possono leggere latest/history dai channel IN collegati, rendendo le porte input un contratto dati e non solo un trigger.
- 2026-06-01: La Flow Map aggiunge il layer `Agent Control Port`: ogni nodo espone una porta virtuale ibrida `agent_control` di tipo `agent-control`, con icona `network_intel_node`, colore cyan e linee doppie per i link di controllo. La porta accetta solo sorgenti AI Agent o `Agent Bridge`. La palette `Processors` include ora `Agent Bridge` (`network_node`), mini node tondo con icona centrale, porta AI a sinistra e due porte a destra: `action` per inviare il payload e `listening` per agganciare l'attesa/risposta del nodo controllato.
- 2026-06-02: La palette `AI Agents` include ora `Task Node`, sorgente operativa dedicata ai job degli agenti. Il nodo emette sulla porta `task` un payload strutturato con objective, context, priority, successCondition, constraints, limits e payload custom, ed e' supportato dal Live Test come starter root.
- 2026-06-02: L'Orchestrator Agent ora usa una pipeline planner/observe: analizza il grafo collegato, chiede un piano JSON al provider AI locale (LM Studio/Ollama), esegue step `run_node` sui source/runtime node, osserva il payload ottenuto, richiede il comando successivo e invia il risultato reale al target collegato (`send_result`). Se il provider AI non risponde, usa un fallback rule-based per route semplici come BTC source -> Preview.
